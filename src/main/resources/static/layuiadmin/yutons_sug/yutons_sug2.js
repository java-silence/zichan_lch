...
layui.define(['jquery', 'table'], function(exports) {
	...
	function getSug(opt) {
        ...
        if(opt.brefore) {
            opt.brefore(opt.id);
            opt.urlBak = sessionStorage.getItem("url")
        }
		//sug下拉框异步加载数据并渲染下拉框
		$.ajax({
			type: "get",
			...
			success: function(data) {
				...
				layui.each(data.data, (index, item) => {
					//if (item[sessionStorage.getItem("inputId")].indexOf(decodeURI(kw)) >= 0) {
						html +=
							"<div class='item' style='padding: 3px 10px;cursor: pointer;' onmouseenter='getFocus(this)' onmousedown='getCon(this);'>" +
							item + "</div>";
					//}
				});
				if (html != "") {
	...
	window.getCon = function(obj) {
        ...
        $("#" + $(".item").parent().prev().attr("id")).delay(100).blur();
	...
	//暴露方法
	exports("yutons_sug2", yutons_sug);
});
