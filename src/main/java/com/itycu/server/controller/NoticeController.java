...
import com.itycu.server.dao.NoticeDao;
import com.itycu.server.dto.NoticeReadVO;
import com.itycu.server.dto.NoticeVO;
...
import com.itycu.server.model.Notice.Status;
...
public class NoticeController {
	...
	public PageTableResponse listNoticeReadVO(PageTableRequest request) {
		...
		//request.getParams().put("status", 0);
		...
		return new PageTableHandler(new CountHandler() {
			...
			public int count(PageTableRequest request) {
				...
				return noticeDao.countNoticeByUser(request.getParams());
			}
		}, new ListHandler() {
			...
			public List<NoticeReadVO> list(PageTableRequest request) {
				...
				return noticeDao.listNoticeByUser(request.getParams(), request.getOffset(), request.getLimit());
			}
		}).handle(request);
	}
	...
	@GetMapping("/listnew")
	public List<Notice> listnew(){
		return noticeDao.listnew();
	}
