...
import com.itycu.server.dto.NoticeReadVO;
...
public interface NoticeDao {
	...
	@Insert("insert into t_notice(userId,title, content, status, createTime, updateTime) values(#{userId},#{title}, #{content}, #{status}, #{createTime}, #{updateTime})")
	int save(Notice notice);

	@Insert("insert into t_notice(title, content, status, createTime, updateTime,userId) values(#{title}, #{content}, #{status}, #{createTime}, #{updateTime},#{userId})")
	int save1(Notice notice);
	...
	int countNoticeByUser(@Param("params") Map<String, Object> params);

	List<NoticeReadVO> listNotice(@Param("params") Map<String, Object> params, @Param("offset") Integer offset,
                                  @Param("limit") Integer limit);

	List<NoticeReadVO> listNoticeByUser(@Param("params") Map<String, Object> params, @Param("offset") Integer offset,
                                  @Param("limit") Integer limit);

	@Select("select * from t_notice t where t.status =1 order by createTime desc")
	List<Notice> listAll();
	@Select("select * from t_notice t where t.status =1 order by createTime desc limit 5")
	List<Notice> listnew();
