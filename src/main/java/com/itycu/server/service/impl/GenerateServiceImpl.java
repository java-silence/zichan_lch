...
import com.itycu.server.utils.TemplateUtil;
...
public class GenerateServiceImpl implements GenerateService {
	...
	private RowMapper<BeanField> beanFieldMapper = new RowMapper<BeanField>() {
		...
		public BeanField mapRow(ResultSet rs, int paramInt) throws SQLException {
			...
			beanField.setIsNullable(rs.getString("is_nullable"));
	...
	public List<BeanField> listBeanField(String tableName) {
		List<BeanField> beanFields = jdbcTemplate.query(
				"select column_name, data_type, column_comment, column_default,is_nullable FROM information_schema.columns WHERE table_name= ? and table_schema = (select database())",
		...
		beanFields.parallelStream().forEach(b -> {
			...
			if ("createby".equals(b.getName())) {
				b.setType(Long.class.getSimpleName());
			}
			if ("updateby".equals(b.getName())) {
				b.setType(Long.class.getSimpleName());
			}
			if ("auditby".equals(b.getName())) {
				b.setType(Long.class.getSimpleName());
			}
			if ("deptid".equals(b.getName())) {
				b.setType(Long.class.getSimpleName());
			}
